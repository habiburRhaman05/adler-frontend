...
export function WorkloadPage() {
  ...
  const [detailsSheet, setDetailsSheet] = useState<WorkloadWeek | null>(null);
  const [detailsOpen, setDetailsOpen] = useState(false);

  // Fetch the editing sheet
  const { data: weekDetail, isLoading: editingLoading } = useWorkloadWeek(
    mode === "edit" && editingSheetId ? editingSheetId : ""
  );
  
  const editingSheet = weekDetail?.data?.week;

  const handleViewDetails = (sheet: WorkloadWeek) => {
    setDetailsSheet(sheet);
    setDetailsOpen(true);
  };

  const handleEditSheet = (sheet: WorkloadWeek) => {
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <h1 className="text-2xl md:text-3xl font-bold mt-1 text-slate-900 tracking-tight">
                  {editingSheet ? `Week ${editingSheet.weekNumber}` : "Week"}
                </h1>
                <p className="text-slate-500 mt-1 font-medium text-sm">
                  {editingSheet?.month}/{editingSheet?.year} &mdash;{" "}
                  {editingSheet ? formatDate(editingSheet.weekStartDate) : ""} → {editingSheet ? formatDate(editingSheet.weekEndDate) : ""}
                </p>
              </div>
            </div>
          ) : (
