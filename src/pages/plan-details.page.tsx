...
import { AssignSheet, type AssignSheetData } from "@/components/plans/assign-sheet";
...
function PlanDetailsPage() {
  ...
  const [assignSheetData, setAssignSheetData] = useState<AssignSheetData | null>(null);
  ...
  return (
    <div className="relative min-h-screen bg-slate-50 text-foreground overflow-hidden dark:bg-slate-950">
      ...
      <div className="mx-auto max-w-[1600px] space-y-4 px-6 py-6">
        ...
        <div className="animate-in fade-in slide-in-from-bottom-2 duration-500 mt-4 h-full relative z-10">
          <CalendarView
            workers={workers}
            plan={plan}
            categories={categories}
            onOpenAssign={(day, start, existing) => setAssignSheetData({ day, start, end: existing?.end || "", existing, categoryId: existing?.categoryId || "", workerId: existing?.workerId || "" })}
          />
        </div>
      </div>
      ...
      <AssignSheet 
        ...
        onAssign={(workerId, categoryId, start, end, existing) => {
          ...
          if (worker && assignSheetData) {
            requestAssign(worker, assignSheetData.day, start, end, existing, categoryId);
          }
        }}
        onDelete={clearSlot}
      />
