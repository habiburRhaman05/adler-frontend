...
import { useCreateWeek } from "@/features/workload/hooks/use-workload";
import type { CreateWeekInput } from "@/features/workload/api/workload.service";
...
export function CreateSheetModal({ open, onOpenChange }: CreateSheetModalProps) {
  ...
  const createMut = useCreateWeek();

  const handleCreate = () => {
    if (!currentWeek) return;
    const weekNumber = weeks.findIndex((w) => w === currentWeek) + 1;
    const data: CreateWeekInput = {
