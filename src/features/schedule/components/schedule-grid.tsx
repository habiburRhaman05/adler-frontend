...
export function ShiftChip({ shift, isViolOpen, onClick, onRemove, onSimulateAccept, onSimulateReject }: ShiftChipProps) {
  ...
  return (
    <div className={baseClass} onClick={onClick}>
      ...
      {isPending && onSimulateAccept && onSimulateReject && (
        <div className="mt-1 flex gap-1 border-t border-blue-200/50 pt-1" onClick={e => e.stopPropagation()}>
          <button 
            className="flex-1 rounded bg-blue-600/10 text-[10px] py-0.5 hover:bg-emerald-100 hover:text-emerald-700"
            onClick={onSimulateAccept}
          >
            Accept
          </button>
          <button 
            className="flex-1 rounded bg-blue-600/10 text-[10px] py-0.5 hover:bg-red-100 hover:text-red-700"
...
interface ScheduleGridProps {
  ...
  onRemoveShift: (staffId: number, dayIdx: number, shiftId: string) => void;
  onSlotClick: (violId: string | null, dayIdx: number, fnKey?: string, needLabel?: string) => void;
  onSimulateStaffResponse: (staffId: number, dayIdx: number, shiftId: string, action: "accepted" | "rejected") => void;
}

export function ScheduleGrid({ days, staffList, grid, violations, onShiftClick, onRemoveShift, onSlotClick, onSimulateStaffResponse }: ScheduleGridProps) {
                ...
                return (
                  <td key={di} className="px-2 py-2 border-l border-slate-100 align-top group/cell relative">
                    ...
                    <div className="flex flex-col gap-2 relative z-10">
                      {shifts.map((s) => {
                        const isViolOpen = !!(s.viol && violations.some(x => 
                          !x.fixed && (
                            (x.kind === "rest" && s.viol === "rest" && st.id === 2) || 
                            (x.kind === "week" && s.viol === "week" && st.id === 4)
                          )
                        ));
                        ...
                        return (
                          <ShiftChip 
                            ...
                            onSimulateAccept={() => onSimulateStaffResponse(st.id, di, s.id, "accepted")}
                            onSimulateReject={() => onSimulateStaffResponse(st.id, di, s.id, "rejected")}
                          />
                        );
                      })}
                    </div>
